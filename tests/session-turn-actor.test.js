...
test('reserveStart marks the error uncertain when durable claim rollback fails', () => {
  const store = new FakeStore(
    { s1: {} },
    { flushResults: [false], clearResults: [false] }
  );
  const registry = createRegistry();
  ...
  assert.throws(
    () => reserve(registry, store, 's1'),
    (error) => {
      assert.equal(error.reason, 'active_turn_claim_flush_refused');
      assert.equal(error.claimState, 'uncertain');
      return true;
    }
  );
  assert.notEqual(store.getActiveTurn('s1'), null);
});

test('reserveStart marks the error not claimed when durable claim rollback succeeds', () => {
  const store = new FakeStore({ s1: {} }, { flushResults: [false, true] });
  const registry = createRegistry();
