...
def resolve_call(self, messages):
    ...
    if any("MIXED_CHILD_TASK" in text for text in users):
    ...
    count = len(results)
    if count in (0, 3):
        call = {"tool_id": "session_spawn", "arguments": {"task": "MIXED_CHILD_TASK " + str(count)}}
    elif count in (1, 4):
        call = {"tool_id": "session_wait", "arguments": {"child_work_id": spawns[-1]}}
    elif count == 2:
        call = ({"tool_id": "read_file", "arguments": {"path": "fixture.txt"}}
                if any("MIXED_APPROVAL" in text for text in users) else
